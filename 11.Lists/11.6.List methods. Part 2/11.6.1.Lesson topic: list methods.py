...
Мы уже познакомились с двумя списочными методами append() и extend(). Первый добавляет в конец списка один новый элемент, а второй расширяет список другим списком. 
...
Если индекс не указан, то метод удаляет и возвращает последний элемент списка. Если список пуст или указан индекс за пределами диапазона, 
...
Метод reverse() меняет порядок элементов на обратный в текущем списке, а срез создает копию списка, в котором элементы следуют в обратном порядке.
...
Примечание. Существует большая разница в работе строковых и списочных методов. Строковые методы не изменяют содержимого объекта, к которому они применяются, 
а возвращают новое значение. Списочные методы (кроме методов index(), count(), copy()), напротив, меняют содержимое объекта, к которому применяются.

Например, когда мы применяем метод lower() к строке original_string, то создаётся новая строка lowercase_string, которая не имеет ничего общего 
со строкой original_string. Более точно будет сказать, что строковый метод вернул новую строку, не изменяя старую:
...
Как мы видим, сама строка original_string не изменилась. Мы просто использовали её для создания строки lowercase_string через метод lower(). 
А lowercase_string действительно создана на основе строки original_string, где каждый символ в нижнем регистре.
Совершенно иная ситуация возникает при работе со списками. Например, при добавлении элемента в список через метод append() мы изменяем наш список, 
не создавая новый. Именно поэтому использование метода append() ничего не возвращает.
...
Как мы видим, original_list изменился. Элемент 11 был добавлен в конец. А вот наша попытка присвоить списку modified_list метод append() 
не увенчалась успехом. Был возвращён объект None (подробнее этот объект будет рассмотрен в курсе для продвинутых). 
